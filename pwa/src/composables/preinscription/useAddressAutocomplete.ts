...
export function useAddressAutocomplete( form: PreInscriptionFormData ) {
	...
	const searchBirthCity = ( query: string, repNum = 0 ) => {
		...
		searchTimeout = setTimeout( async () => {
			...
			if ( repNum === 0 ) {
				suggestions.birthCity = results;
			} else if ( repNum === 1 ) {
				suggestions.rep1BirthCity = results;
			} else if ( repNum === 2 ) {
				suggestions.rep2BirthCity = results;
			}
		}, 250 );
	};
	...
	const searchAddress = ( query: string, repNum = 0 ) => {
		...
		searchTimeout = setTimeout( async () => {
			...
			if ( repNum === 0 ) {
				suggestions.address = results;
			} else if ( repNum === 1 ) {
				suggestions.rep1Address = results;
			} else if ( repNum === 2 ) {
				suggestions.rep2Address = results;
			}
		}, 250 );
	};
