...
export function formatMoveInFrench(
	...
	if ('promotion' in parsedMove && parsedMove.promotion) {
		const promoRole = ROLE_NAMES_FR[parsedMove.promotion] || parsedMove.promotion;
		texte += ` (${promoRole})`;
	}
 ...
 */
export function parseOuvreBoiteMiniPgn(rawPgn: string, index: number = 0): CarteOuvreBoite {
	...
	const orientation: 'white' | 'black' = pos.turn === 'white' ? 'white' : 'black';
	...
	for (const s of rawShapeMatch.shapes) {
		if (!allShapes.some((exist) => exist.orig === s.orig && exist.dest === s.dest && exist.brush === s.brush)) {
			allShapes.push(s);
		}
	}
	...
	if (game && game.moves && game.moves.children.length > 0) {
		...
		const { cleanedText: mainExplication, shapes: mainShapes } = extractShapesAndText(mainComments);
		...
		for (let i = 1; i < game.moves.children.length; i++) {
			...
			const { cleanedText: varExplication, shapes: varShapes } = extractShapesAndText(varComments);
