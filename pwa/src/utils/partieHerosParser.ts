...
import { parsePgn, type PgnNodeData } from 'chessops/pgn';
...
export function parsePartieHerosPgn(
	...
	let node: { data?: PgnNodeData; children: Array<{ data: PgnNodeData; children: any[] }> } = root as any;
	...
	while (node) {
		...
		if (node.children.length > 1) {
			...
			stages.push({
				...
				orientation: getActiveColorFromFen(makeFen(currentPos.toSetup())),
				shapes: qcmShapes,
