...
describe('apprentissageStore - selective sync & modified timestamp', () => {
  ...
  it('preserves cached content if modified timestamp is identical', async () => {
    await app.runWithContext(async () => {
      ...
      queryClient.setQueryData(['parcours', identityId], [
        {
          id: 1,
          titre: 'Cours 1',
          niveau: 1,
          playlist: [
            { id: 101, type: 'roi_exercice', titre: 'Exercice 1', modified: '2026-09-14 10:00:00' },
          ],
        },
      ]);
  ...
  it('prefetches only when item is not in cache or modified has changed', async () => {
    await app.runWithContext(async () => {
      ...
      queryClient.setQueryData(['parcours', identityId], [
        {
          ...
          playlist: [
            { id: 101, type: 'roi_exercice', titre: 'Exercice 1', modified: '2026-09-14 10:00:00' },
            { id: 102, type: 'roi_exercice', titre: 'Exercice 2', modified: '2026-09-14 11:00:00' },
          ],
        },
      ]);
      ...
      expect((safeFetch as any).mock.calls[0][0]).toContain('/roi/v1/contenu/102');
    });
  });
});
