...
import { parseAssociPlanSinglePgn, parseAssociPlanPaires } from '@/utils/associPlanParser';
...
1. Ne5 { Le Cavalier se centralise tout simplement. } 1... Qc7 2. f4 *`;
	...
	it('extrait correctement la FEN, l\'orientation, les shapes et la description initiale', () => {
		...
		expect(result.fen).toBe('r2q1rk1/pb3ppp/1pnbpn2/2pp4/3P4/2PBPNB1/PP1N1PPP/R2Q1RK1 w - - 0 1');
		expect(result.couleurJoueur).toBe('white');
		expect(result.description).toBe('Voici une position avec beaucoup de pièces. Beaucoup de coups sont possibles.');
		expect(result.shapes).toHaveLength(3);
		expect(result.shapes[0]).toEqual({ orig: 'e5', brush: 'green' });
		expect(result.shapes[1]).toEqual({ orig: 'd2', dest: 'f3', brush: 'green' });
		expect(result.shapes[2]).toEqual({ orig: 'f3', dest: 'e5', brush: 'green' });
	});
