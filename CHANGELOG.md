...
- **Célébration Visuelle & Sensorielle de Fin d'Exercice (`useCelebration.ts`, `canvas-confetti`, `@capacitor/haptics`, `ContenuPage.vue`, `SeriesCardFooter.vue`, `useCelebration.spec.ts`)** :
  ...
  - **Temporisation d'apparition de 1s (`SeriesCardFooter.vue`)** : Dès la résolution, le badge de victoire s'affiche immédiatement avec les confettis, et les boutons d'action (« Cours » et « Exercice suivant ») n'apparaissent qu'après un court délai (1000ms), évitant tout clic précipité et laissant le temps d'apprécier la réussite.
