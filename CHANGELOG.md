...
- **Nouvelle Variante « Notation » pour les Exercices Type 14 (Cap ou pas cap ?) (`CapOuPasCapViewer.vue`, `TypeCapOuPasCap.vue`, `TypeCapOuPasCap.spec.ts`, `README.md`, `USING.md`)** :
  ...
  - **Validation Intelligente avec Casse Stricte** : Exigence d'une notation rigoureuse (initiale majuscule pour les pièces ex: `Tc2`, minuscules pour les cases et pions ex: `c3`), validation instantanée (coche verte / croix rouge) et prise en charge des pièces multiples identiques (ex: deux cavaliers dont les coordonnées peuvent être saisies dans n'importe quel ordre).
  - **Couverture de Tests Unitaires Vitest** : Ajout de tests unitaires dédiés dans `TypeCapOuPasCap.spec.ts` validant l'extraction de pièces, le rejet des mauvaises casses, la validation stricte, les pièces multiples et le succès de l'exercice.
...
- **Prise en Charge des Vidéos Pédagogiques & Ergonomie Unifiée (`VideoReader.vue`, `useYouTubePlayer.ts`, `ContenuPage.vue`, `SeriesCardFooter.vue`, `stringUtils.ts`, `apprentissage.ts`)** :
  ...
  - **Scaffold Mobile & Footer Fixe (`SeriesCardFooter`)** : Traitement de la vidéo comme une carte unique (`currentCard: 1`, `totalCards: 1`) téléportée dans le footer fixe au bas de l'écran, éliminant tout panneau de succès résiduel sous la vidéo.
  - **Correction du Déclenchement Intempestif des Confettis (`isAlreadyCompleted`)** : Ajout de la prop `isAlreadyCompleted` dans `SeriesCardFooter.vue`. Lorsqu'une vidéo ou un exercice déjà validé est réouvert, aucun confetti n'est tiré et les boutons de navigation (« Cours » et « Terminer le cours » / « Suivant ») sont affichés immédiatement sans temporisation.
...
- **Stabilisation du Footer Fixe et Surélévation Safe Area Mobile (`ContenuPage.vue`, `SeriesCardFooter.vue`, `variables.css`, `SeriesCardFooter.spec.ts`, `README.md`, `USING.md`)** :
  ...
  - **Gestion résiliente des marges de sécurité mobiles (Safe Areas iOS / Android)** : Remplacement de l'ancien padding fixe par une formule `max()` garantissant une surélévation confortable (`max(16px, calc(12px + env(safe-area-inset-bottom, 0px)), calc(12px + var(--ion-safe-area-bottom, 0px)))`). Le footer n'est plus incrusté au bord de l'écran lorsque la safe area est à 0px (navigateur Safari ou Android avec 16px minimum garanti), et s'élève à 46px sur les iPhone équipés d'une encoche ou du Home Indicator (34px de safe area + 12px de marge de confort tactile sous les boutons). Sécurisation des variables d'environnement dans `variables.css` avec fallbacks explicites à `0px`.
...
- **Mélange Aléatoire des Choix QCM du Type 4 (`partieHerosParser.ts`, `TypePartieHeros.vue`, `partieHerosParser.spec.ts`, `TypePartieHeros.spec.ts`, `USING.md`)** :
  ...
  - **Couverture de tests unitaires** : Création de la suite de tests `partieHerosParser.spec.ts` validant le mélange Fisher-Yates (avec RNG déterministe) et mise à jour de `TypePartieHeros.spec.ts` pour cibler dynamiquement le bon choix par son coup sans dépendre d'un index fixe.
- **Suppression de la Double Célébration & Report de la Victoire au Terme du PGN (`TypePartieHeros.vue`, `PgnViewer.vue`, `ABCDaireTactiqueViewer.vue`, `SeriesCardFooter.vue`, `TypePartieHeros.spec.ts`, `TypeABCDaire.spec.ts`, `SeriesCardFooter.spec.ts`)** :
  - **Type 4 (Partie du Héros) - Footer unique et fin de partie** : Remplacement des deux instances distinctes de `SeriesCardFooter` (auparavant conditionnées par `v-if="etapeActuelle.type === 'pgn'"` et `v-else-if="etapeActuelle.type === 'qcm'"`) par un footer unique et persistant piloté par des propriétés calculées réactives (`isCurrentStageSolved`, `currentStageFeedback`, `currentStagePendingHint`). Cela élimine le cycle de destruction/remontage du footer lors du passage du dernier QCM au PGN final qui déclenchait une célébration parasite à l'affichage du dernier PGN. La célébration unique et le message `🎉 Exercice réussi !` sont désormais déclenchés exclusivement lorsque tous les coups du PGN final ont été déroulés.
  - **Type 3 (ABCDaire Tactique) - Éradication du `@finished` prématuré** : Correction dans `PgnViewer.vue` du test de complétion au chargement (`loadPgnData`), en supprimant la condition sur `!historyState?.isEnabled`. Le composant échiquier initialisant son historique de manière asynchrone, ce test provoquait l'émission immédiate de l'événement `@finished` dès l'apparition du PGN d'explication, causant une première célébration prématurée dès le coup joué puis une seconde à la fin du PGN.
  - **Conditionnement strict du footer de série (`SeriesCardFooter.vue`)** : Verrouillage de la métamorphose de fin d'exercice (`isFinalCompleted`) à `!props.disabled`. Tant que les explications PGN ne sont pas intégralement consultées, le bouton reste désactivé (`disabled: !isPgnFinished`) avec le feedback intermédiaire `« Bravo ! Coup correct. »`.
  - **Couverture de tests unitaires** : Enrichissement des suites `TypePartieHeros.spec.ts`, `TypeABCDaire.spec.ts` et `SeriesCardFooter.spec.ts` avec mock de `fireExerciseCelebration` (`useCelebration`), garantissant 0 appel intermédiaire et un appel unique et final au terme de l'exercice.
...
- **Architecture Scaffold d'Exercice & Footer Fixe Universel (`SeriesCardFooter.vue`, `ContenuPage.vue`, `shared-components.scss`, `TypePartieHeros.vue`, `PgnViewer.vue`, `QcmViewer.vue`, `ExerciseHeader.vue`, `README.md`)** :
  ...
  - **Réinitialisation automatique du scroll (`ion-content`)** : Appel systématique de `scrollToTop(0)` lors du chargement de chaque nouvel exercice pour garantir un affichage dès le sommet de la page.
  - **Zone centrale scrollable (`<ion-content>`)** : L'en-tête d'exercice, l'échiquier et les interactions (coups, boutons QCM, commentaires) résident dans la zone centrale qui active un ascenseur fluide uniquement en cas de dépassement, sans jamais repousser le footer hors de l'écran.
  - **Confort de lecture et stabilisation de la bulle de commentaires PGN** : Extension adaptative de la zone de commentaires (`min-height: 42px; max-height: 120px;`) avec défilement fluide `overflow-y: auto`, permettant d'afficher confortablement jusqu'à 5 lignes de texte pédagogique sans être tronqué, combinée à une réservation minimale (`.comment-empty`, 42px) pour éliminer les sauts d'interface lors des coups sans commentaire.
  - **Harmonisation et compacité QCM (Type 1 et Type 4)** : Échiquier canonique calibré à `max-width: min(100%, 38dvh, 440px)`, boutons de choix compacts (38px, gap 6px) et padding de carte allégé (8px 10px) pour un look & feel parfaitement uniforme.
- **Célébration Visuelle & Sensorielle de Fin d'Exercice (`useCelebration.ts`, `canvas-confetti`, `@capacitor/haptics`, `ContenuPage.vue`, `SeriesCardFooter.vue`, `useCelebration.spec.ts`)** :
  - **Pluie de confettis festifs (`canvas-confetti`)** : Déclenchement automatique d'un feu d'artifice de confettis en double gerbe latérale dès la résolution complète de l'exercice (avec `zIndex: 99999` et dispersion renforcée pour ordinateur).
  - **Temporisation étendue (2,2s) & Persistance du feedback (`SeriesCardFooter.vue`)** : Le bandeau de victoire vert `🎉 Exercice réussi !` s'affiche immédiatement et reste affiché en permanence, tandis que le badge de victoire est maintenu 2,2 secondes avant l'apparition en douceur des boutons d'action (« Cours » et « Exercice suivant »), laissant pleinement le temps de savourer la réussite.
  - **Bouton d'avancement pulsant visible (`pulseScale`)** : Animation `transform: scale(1.08)` et `filter: brightness(1.2)` sur le bouton vert « Exercice suivant » garantissant une pulsation spectaculaire et immanquable sur écran d'ordinateur comme sur mobile.
  ...
  - **Couverture de tests unitaires (`useCelebration.spec.ts`, `SeriesCardFooter.spec.ts`)** : Validation des déclenchements de confettis, de l'haptique et de la temporisation.
