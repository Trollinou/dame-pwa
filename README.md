...
- **DAME-PWA (Client-Side)** : Moteur d'interprétation interactive via `fenUtils.ts`, `partieHerosParser.ts`, `eg-chessboard`, `chessops` et `VideoReader.vue` :
  ...
  - Déroulement en série de 5 mini-situations pour Cap ou pas Cap ? (Type 14) : support des variantes `qcm_multiple`, `qcm_oui_non`, `move` et `notation` (apprentissage du repérage et notation française des pièces avec saisie textuelle et auto-validation), sélecteurs neutres tactiles OUI/NON, masquage initial puis révélation complète des shapes et navigation `SeriesCardFooter`.
